...
function Dashboard({ supabase }) {
  ...
  const handleReset = async () => {
    try {
      console.log('Resetting trades for user:', userId);
      const { data, error } = await supabase
        .from('trades')
        .select('*')
        .eq('user_id', userId)
  ...
  if (loading) {
    return (
      <div className={`container py-20 text-center ${darkMode ? 'dark' : ''}`}>
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
    ...
    <div className={`container py-20 ${darkMode ? 'dark' : ''}`}>
      ...
      {error && (
        <motion.div
          ...
          transition={{ duration: 0.5 }}
        >
          <p className="text-red-500 dark:text-red-400 text-lg" role="alert">{error}</p>
        </motion.div>
      )}
      {!userId ? (
        <motion.div
          ...
          transition={{ duration: 0.5 }}
        >
          <p className="text-red-500 dark:text-red-400 text-lg" role="alert">
